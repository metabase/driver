...
(defn unprepare-date-with-iso-8601-fn
  ...
  [iso-8601-fn, ^Date value]
  (hformat/to-sql
   (hsql/call
    iso-8601-fn
    (hx/literal
     (tformat/unparse (tformat/formatters :date-time) (tcoerce/from-date value))))))
...
(defmethod unprepare-value [:sql Time] [driver, ^Time value]
  (hformat/to-sql (hx/->time (hsql/raw (unprepare-value driver (tcoerce/to-sql-time value))))))

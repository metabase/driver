...
(defmulti field->identifier
  ...
  implementation returns a keyword generated by from the components returned by
  `qp-interface/field-qualified-name-components`. Other drivers like BigQuery need to do additional qualification,
  e.g. the dataset name as well. (At the time of this writing, this is only used by the SQL parameters implementation;
  in the future it will probably be used in more places as well.)"
  {:arglists '([driver field])}
  driver/dispatch-on-initialized-driver
  :hierarchy #'driver/hierarchy)

(defmethod field->identifier :sql [_ field]
  (apply hsql/qualify (qp-interface/field-qualified-name-components field)))
...
(defmulti ->honeysql
  ...
  (fn [driver x]
    [(driver/dispatch-on-initialized-driver driver) (dispatch/dispatch-on-clause-or-model-or-class x)])
  :hierarchy #'driver/hierarchy)
...
(defmethod ->honeysql [:sql :model/Field]
  ...
  (let [qualifiers (if *table-alias*
                     [*table-alias*]
                     (let [{schema :schema, table-name :name} (qp-interface/table table-id)]
                       [schema table-name]))
...
(defmethod ->honeysql [:sql :field-id]
  [driver [_ field-id]]
  (->honeysql driver (qp-interface/field field-id)))
...
(s/defn field-clause->alias
  ...
  ([driver, field-clause :- mbql.s/Field, unique-name-fn :- (s/pred fn?)]
   (when-let [alias (mbql.u/match-one field-clause
                      ...
                      [:field-id id]                (field->alias driver (qp-interface/field id)))]
  ...
  (let [honeysql-ags (for [ag aggregations]
                       [(->honeysql driver ag)
                        (->honeysql driver (hx/identifier
                                            :field-alias
                                            (driver/format-custom-field-name driver (qp-interface/aggregation-name ag))))])]
...
(defmethod join-source :sql
  ...
  (binding [*table-alias* nil]
    (if source-query
      ...
      (->honeysql driver (qp-interface/table source-table)))))
...
(defmethod apply-top-level-clause [:sql :source-table]
  ...
  (h/from honeysql-form (->honeysql driver (qp-interface/table source-table-id))))
...
(defn- format-honeysql [driver honeysql-form]
  (try
    ...
    (catch Throwable e
      (log/error (colorize/red
                  (str (tru "Invalid HoneySQL form:")
                       "\n"
                       (with-out-str (pprint/pprint honeysql-form)))))
      (throw e))))
...
(s/defn build-honeysql-form
  ...
  (let [honeysql-form (apply-clauses driver {} inner-query)]
    (when-not (qp-interface/disable-logging?)
      (log/debug
       (tru "HoneySQL Form:")
       "\n"
       (colorize/cyan
        (with-out-str
          (pprint/pprint honeysql-form)))))
    honeysql-form))

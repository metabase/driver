...
(defn parse-date-as-string
  ...
  (let [date-string (.getString rs i)]
    (if-let [parsed-date (common/str->date-time date-string (.getTimeZone cal))]
      parsed-date
  ...
  (let [sql              (str "-- " remark "\n" sql)
        [columns & rows] (cancelable-run-query
                          ...
                          {:identifiers    identity
                           ...
                           :max-rows       max-rows})]
    {:rows    (or rows [])
     :columns (map mbql.u/qualified-name columns)}))
...
(defn- set-timezone!
  ...
  {:pre [(re-matches #"[A-Za-z\/_]+" report-timezone)]}
  (let [format-string (set-timezone-sql driver)
        sql           (format format-string (str \' report-timezone \'))]
    (log/debug (colorize/green (tru "Setting timezone with statement: {0}" sql)))
...
(defn execute-query
  ...
  (let [query (assoc query
                :remark   (qp-interface/query->remark outer-query)
                :max-rows (or (mbql.u/query->max-rows-limit outer-query) driver/absolute-max-results))]
    (do-with-try-catch
      (fn []
        (let [db-connection (sql-jdbc.conn/db->pooled-connection-spec (qp-interface/database))]

...
(def absolute-max-results
  "Maximum number of rows the a query should ever return.
  ...
  (binding [*use-context-classloader* true]
    (let [expected-ns (driver->expected-namespace driver)]
      ...
      (locking require-lock
        (log/debug
         (trs "Loading driver {0} {1}" (colorize/blue driver) (apply list 'require expected-ns require-options)))
        (try
...
(defn register!
  ...
  (when-not *compile-files*
    ...
    (let [derive! (partial alter-var-root #'hierarchy derive driver)]
      ...
      (when-not abstract?
        (derive! ::concrete))
      (doseq [parent (if ((some-fn sequential? set?) parent)
                       parent
                       [parent])
              :when  parent]
    ...
    (log/info
     ...
     (if (seq (filter some? (if ((some-fn sequential? set?) parent)
                              parent
                              [parent])))
  ...
  (when-not *compile-files*
    ...
    (when-not (initialized? driver)
      ...
      (locking initialization-lock
        ...
        (when-not (initialized? driver)
          (log/info (colorize/yellow (trs "Initializing driver {0}..." driver)))
...
(defmethod date-interval ::driver [_ unit amount]
  (mbql.u/relative-date unit amount))
...
(defmulti ^DateTime current-db-time
  ...
  ;; TODO - this should probaby return one of the new Java 8 time types instead
  {:arglists '([driver database])}
  dispatch-on-initialized-driver
  :hierarchy #'hierarchy)
